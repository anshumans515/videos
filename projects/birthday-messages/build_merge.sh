#!/usr/bin/env bash
# Merge the 14 birthday-message clips into one 1080x1920 / 30fps video.
#
...
ffmpeg -nostdin -v error -y -f concat -safe 0 -i "$HERE/concat.txt" \
  -c copy -movflags +faststart "$HERE/public/messages-merged.mp4"
...
ffprobe -v error -show_entries stream=codec_type,width,height,r_frame_rate \
  ...
  "$HERE/public/messages-merged.mp4"
echo "--- loudness ---"
ffmpeg -nostats -i "$HERE/public/messages-merged.mp4" -af ebur128=peak=true -f null - 2>&1 \
  | grep -E "^ +(I|Peak):"
